...
function ScreenBlock({ children }: { children: ReactNode }) {
  return <div className="w-136.5 space-y-5">{children}</div>;
}

function Header({ icon, children }: { icon: string; children: ReactNode }) {
  return (
    <div className="w-full border-2 border-primary-yellow rounded-2xl py-5.5 flex justify-center items-center gap-6 bg-white">
      <img src={icon} alt="" />
